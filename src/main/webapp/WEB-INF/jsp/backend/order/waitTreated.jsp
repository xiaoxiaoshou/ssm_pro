    ...
    <table class="layui-table">
        ...
        <c:forEach items="${orders}" var="order">
            <tr>
                ...
                <th>
                    <a title="完成就诊" onclick="modifyStatus(this,'${order.id}')" href="javascript:;">
...
<script>
    ...
    function modifyStatus(obj,id){
        layer.confirm('你确认该用户已经完成就诊吗？',function(index){
            //发异步修改数据
            $.post(
                '${mybasePath}backend/modifyOrderStatus?id='+id,
                function (result) {
                    if(result.status==1){
                        $(obj).parents("tr").remove();
                        layer.msg('完成!',{icon:1,time:1000});
                        window.location.href="${mybasePath}backend/skipWaitTreated"
                    }
                }
            )
        });
