...
import cn.sfturing.dao.front.CommonUserDao;
import cn.sfturing.dao.front.OrderRecordsDao;
	...
	public boolean sendEmailCheck(CommonUser commonUser, OrderRecords orderRecords) {
		...
		if (isSuccess == true) {
			...
			SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			 commonUserDao.sendVerification(email, 1,df.format(new Date()));
